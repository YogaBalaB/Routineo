...
async def checkin_habit(habit_id: str, current_user: dict = Depends(get_current_user)):
    ...
    habit = result[0]
    # ✅ Use date-only string YYYY-MM-DD for history — no timezone issues
    ...
    if today not in history:
        ...
        if last_check:
            # last_check_in is stored as YYYY-MM-DD — safe to parse directly
        ...
        update = {
            ...
            # ✅ Store as date-only string — avoids JS timezone parsing issue
            "last_check_in": today,
        }
...
# ✅ /bulk MUST be before /{habit_id} to avoid route conflict
@router.delete("/bulk")
async def delete_habits_bulk(
